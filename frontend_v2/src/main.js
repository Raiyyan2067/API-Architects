...
async function parseOrderXmlForLines(file) {
    try {
        ...
        if (doc.querySelector('parsererror')) { clearLineInputs(); return; }

        // getElementsByTagName with prefixed names works across all browsers
        // for XML parsed by DOMParser — try both prefixed and unprefixed
        const getTag = (node, tag) => {
            let els = node.getElementsByTagName('cac:' + tag);
            if (!els.length) els = node.getElementsByTagName(tag);
            return els;
        };
        const getTagText = (node, tag) => {
            let els = node.getElementsByTagName('cbc:' + tag);
            if (!els.length) els = node.getElementsByTagName(tag);
            return els.length ? (els[0].textContent || '').trim() : '';
        };

        const orderLines = getTag(doc, 'OrderLine');
        if (!orderLines.length) { clearLineInputs(); return; }

        const lines = [];
        for (const ol of orderLines) {
            const liList = getTag(ol, 'LineItem');
            if (!liList.length) continue;
            const li = liList[0];

            let qtyEl = li.getElementsByTagName('cbc:Quantity')[0];
            if (!qtyEl) qtyEl = li.getElementsByTagName('Quantity')[0];

            const id   = getTagText(li, 'ID');
            const itemNodes = getTag(li, 'Item');
            const item = itemNodes.length ? itemNodes[0] : li;
            const name = getTagText(item, 'Name') || getTagText(item, 'Description') || `Line ${id}`;
            const qty  = qtyEl ? (qtyEl.textContent || '0').trim() : '0';
            const unit = qtyEl ? (qtyEl.getAttribute('unitCode') || 'EA') : 'EA';

            lines.push({ id, name, qty: parseFloat(qty) || 0, unit });
        }

        if (!lines.length) { clearLineInputs(); return; }
...
window.startEditMode = async function(despatchId) {
    ...
    if (!banner) {
        ...
        banner.style.cssText = 'background:var(--amber);color:#000;padding:10px 14px;border-radius:8px;font-size:13px;margin-bottom:16px;display:flex;justify-content:space-between;align-items:center;';
    ...
    if (!MOCK_MODE) {
        try {
            const res = await fetch(`${API_BASE}/ubl/v3/despatch-advice/id/${despatchId}/source-order`, {
                headers: { 'Authorization': `Bearer ${getToken()}` }
            });
            if (res.ok) {
                const xml = await res.text();
                const blob = new Blob([xml], { type: 'application/xml' });
                const file = new File([blob], `order_${despatchId}.xml`, { type: 'application/xml' });
                selectedFile = file;
        ...
        } catch (_) { /* non-fatal — user can upload a new order */ }
    }
}
...
document.getElementById('generate-form').addEventListener('submit', async (e) => {
    ...
    if (!isEdit && !selectedFile) { showToast('Please upload an Order XML file', 'error'); return; }
    ...
    btn.disabled = true;
    btn.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="animation:spin 1s linear infinite"><path d="M21 12a9 9 0 1 1-6.22-8.56"/></svg> ' + (isEdit ? 'Updating…' : 'Generating…');
    ...
    try {
        ...
        if (MOCK_MODE) {
            await mockDelay(900);
            const orderId = isEdit ? editModeId : 'ORD-' + String(mockStore.documents.length + 1).padStart(3, '0');
            uuid     = crypto.randomUUID();
            xml      = MOCK_XML(orderId, carrier, date, notes);
            filename = `Despatch_${orderId}_${uuid}.xml`;
            if (!isEdit) mockStore.documents.unshift({ uuid, despatch_id: orderId, created_at: new Date().toISOString() });
        } else {
            ...
            if (isEdit) {
                ...
                res = await fetch(`${API_BASE}/ubl/v3/despatch-advice/id/${editModeId}/edit`, {
                    method: 'PUT',
                    headers: { 'Authorization': `Bearer ${getToken()}` },
                    body: fd,
                });
        ...
        showToast(isEdit ? 'Despatch updated!' : 'Despatch Advice generated!', 'success');
        ...
        btn.disabled = false;
        btn.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z"/></svg> ' + (editModeId ? 'Update Despatch' : 'Generate Despatch Advice');
    }
});
...
document.getElementById('validate-btn').addEventListener('click', async () => {
    ...
    try {
        ...
        if (MOCK_MODE) {
            ...
            const res = await fetch(`${API_BASE}/ubl/v3/despatch-advice/validate`, {
                method: 'POST',
                body: fd
            });
            const data = await res.json().catch(() => ({}));
